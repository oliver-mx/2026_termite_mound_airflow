...
!!! note
        In `r-h_simulations.jl` and `optimisation.jl` we solve the poroblems with a smaller step size and increased polynomial degree in order to avoid any numerical instabilites that could arise.
        Therefore, these two scrips run significantly slower due to this precautious measure.
...
!!! note
        We provide already the output generated by `r-h_simulations.jl`.
