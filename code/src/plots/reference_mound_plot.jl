# Creates plots of the `reference_mound_simulation.jl` script
# 
...
if isdir(output_dir) == false
    error("Run reference_mound_simulation.jl first!")
end
...
include("../callbacks/update_velocity_plot_24h.jl")
include("../../out/reference_mound/output.jl"); export read_output
...
@muladd @inline function plot_solution(plot_dir, nodes, time, rho, v, T, Ti, Tu, p0, equations)
    ...
    plot!(p_rho, xlabel="", ylabel="", zlabel="",
            camera=(0, 90.001), colorbar_title=" ", yticks=[0,4,8,12,16,20,24], zticks=nothing, colorbar = true)
...
@muladd @inline function plot_experiment(output_dir, nodes, time, rho, v, T, Ti, Tu, equations)
    ...
    p_vel = plot(dead_vel_x_data, dead_vel_y_data, 
        ...
        linecolor=:green, 
        xlabel="t (h)", ylabel="Velocity (m/s)", label = "Dead mounds")
    ...
    p_Tnest = plot(t, T_air.(t, equations) .- 273.15,
                ...
                xlabel="t (h)", ylabel="Temperature (°C)", label = "T_air", xticks = [0,4,8,12,16,20,24], ylim=(19.0, 34.0), pos = :top)
...
@muladd @inline function plot_profiles(output_dir, nodes, t, equations)
    ...
    #-----------------------
    p_h = plot(nodes, [height_profile(x, equations) for x in range(0,1,length(nodes))],
                        title = " ", legende = false, size=(700, 550),
