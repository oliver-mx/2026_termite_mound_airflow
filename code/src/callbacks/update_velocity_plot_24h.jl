...
"""
    UpdateVelocityCallback - plot_24h
...
mutable struct UpdateVelocityCallback{ODE_solver, Out_dir, Vis_count, Vel_count, L2_count, L2_nodes, L2_error}
    solver::ODE_solver
    output_dir::Out_dir
    a::Vis_count
    b::Vel_count
...
function Base.show(io::IO, ::MIME"text/plain",
    ...
    if get(io, :compact, false)
        ...
        setup = [
            "Solver" => update_velocity_callback.solver
        ]
        Trixi.summary_box(io, "UpdateVelocityCallback", setup)
    end
end

function UpdateVelocityCallback(solver, output_dir ;a=1::Int, b=1.0, c=1::Int, d=zeros(100,), e=0.0)
    # Convert plain real numbers to functions for unified treatment
    solver_conv = isa(solver, Real) ? Returns(solver) : solver
    out_conv = isa(output_dir, Real) ? Returns(output_dir) : output_dir
    ...
    update_velocity_callback = UpdateVelocityCallback{typeof(solver_conv), typeof(out_conv), typeof(a_conv), typeof(b_conv), typeof(c_conv), typeof(d_conv), typeof(e_conv)}(solver_conv, out_conv, a_conv, b_conv, c_conv, d_conv, e_conv)
...
@inline function (update_velocity_callback::UpdateVelocityCallback)(integrator)
    ...
    b_now, c_now, d_now, e_now = if t_prev ≤ 0 && t_now > 0 
        (1.0, 1, zeros(100,), 0.0)
    else
        (b_now, c_now, d_now, e_now)
    end
    a_new, b_new, c_new, d_new, e_new = write_plot_data(update_velocity_callback.output_dir, a_now, b_now, c_now, d_now, e_now, integrator, tspan, nodes, t, ti, 151, rho, v1, v1_prev, T, Ti, Tu, p0, equations)
...
@inline function write_plot_data(output_dir, a, b, c, d, e, integrator, tspan, nodes, t, ti, max_visnodes, rho, v1, v1_prev, T, Ti, Tu, p0, equations::TermiteMoundEquations1D)
    #-----------------------
    L2_index = range(0.0, tspan[end], 100)
    ...
    b, d, e =   if integrator.t == tspan[end]
                    ...
                    L2_LI = LinearInterpolation2(x_nodes, y_nodes, equations)
